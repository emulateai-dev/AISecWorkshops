...
SETUP_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
PWD_COMPOSE="$SETUP_ROOT/docker/erpnext/pwd.yml"
DEFAULT_COMPOSE="$SETUP_ROOT/docker/erpnext/docker-compose.yml"
ENV_FILE="${ERPNext_ENV_FILE:-$SETUP_ROOT/docker/erpnext/.env}"
ENV_TEMPLATE="$SETUP_ROOT/docker/erpnext/.env.example"
...
compose_file=""
if [ -f "$PWD_COMPOSE" ]; then
    compose_file="$PWD_COMPOSE"
elif [ -f "$DEFAULT_COMPOSE" ]; then
    compose_file="$DEFAULT_COMPOSE"
else
    echo "No compose file found (expected pwd.yml or docker-compose.yml) under setup/docker/erpnext." >&2
    exit 1
fi
...
# Run migrate and ensure Guest enabled
