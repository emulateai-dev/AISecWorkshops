...
# ── Secret Discovery (informational) ──────────────────────────
info "Checking ~/.secrets/ for existing keys..."
for key in OPENAI_API_KEY GROQ_API_KEY XAI_API_KEY GROK_API_KEY; do
  if [[ -f "$HOME/.secrets/${key}.txt" ]]; then
    success "Found ${key}.txt"
  fi
done
echo ""
...
case "\${PROVIDER}" in
  ...
  xai|grok)
    ...
    MODEL="\${FOLLY_MODEL:-grok-2-latest}"
    if [[ -n "\${XAI_API_KEY:-}" ]]; then
      API_KEY="\${XAI_API_KEY}"
    elif [[ -f "\$HOME/.secrets/XAI_API_KEY.txt" ]]; then
      API_KEY="\$(cat "\$HOME/.secrets/XAI_API_KEY.txt")"
    elif [[ -f "\$HOME/.secrets/GROK_API_KEY.txt" ]]; then
      API_KEY="\$(cat "\$HOME/.secrets/GROK_API_KEY.txt")"
    else
      echo "❌ XAI_API_KEY or GROK_API_KEY not set (checked ENV and ~/.secrets/)."
      exit 1
    fi
    ;;
  ollama)
...
echo ""
echo "       # xAI / Grok"
echo "       FOLLY_PROVIDER=xai ${START_SCRIPT}"
echo ""
